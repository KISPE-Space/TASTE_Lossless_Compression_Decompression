...
- Internal Memory for the ZLib library. Zlib tends to use around ~400 kBytes of memory to perform its compression (decompression tends to be less costly). The maximum memory available to Zlib is defined by the compile time constant "STATIC_POOL_SIZE". This defaults to 384kBytes (the number is specified in bytes). If the compression fails, it may be that Zlib requires more memory, adjusting this compile time contant can be done to provide it further resource.
